...
const Gravity = forwardRef<GravityRef, GravityProps>(
  (
    ...
    const initializeRenderer = useCallback(() => {
      ...
      // Matter.js's Mouse attaches a `wheel` listener with preventDefault on
      // the canvas, swallowing page scroll while the cursor is over the
      // playground. Detach it — we don't use wheel events for physics.
      const canvasEl = render.current.canvas;
      // matter-js stores its handler reference as `mousewheel`
      // (see matter.js Mouse.create)
      const wheelHandler = (mouse as unknown as { mousewheel?: EventListener })
        .mousewheel;
      if (wheelHandler) {
        canvasEl.removeEventListener("wheel", wheelHandler);
        canvasEl.removeEventListener("mousewheel", wheelHandler);
        canvasEl.removeEventListener("DOMMouseScroll", wheelHandler);
      }
      // Make sure the canvas never blocks vertical scrolling on touch devices.
